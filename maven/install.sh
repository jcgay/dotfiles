...
(cd $MAVEN_COLOR && ln -s $MAVEN_COLOR/maven-color-agent-0.3.jar maven-color-agent.jar)
...
wget -P $MAVEN_NOTIFIER http://dl.bintray.com/jcgay/maven/com/github/jcgay/maven/maven-notifier/0.7/maven-notifier-0.7.zip
(cd $MAVEN_NOTIFIER && unzip maven-notifier-0.7.zip && rm maven-notifier-0.7.zip)
