...
> **Edit the `##template` file, never the rendered file directly** — the next
> `yadm alt` would overwrite your changes.

Templates don't only branch on class. They also branch on the machine's
**architecture** (`yadm.arch`) when a path differs between Apple Silicon and
Intel — most notably the Homebrew prefix (`/opt/homebrew` on `arm64`,
`/usr/local` on `x86_64`). See
[`.config/ghostty/config##template`](.config/ghostty/config##template), which
selects the right `fish` binary path per machine:
...
> ℹ️ **Note:** yadm's built-in template processor only understands the
> **multi-line** `{% if %}` / `{% else %}` / `{% endif %}` form (each directive
> on its own line). Inline conditionals are *not* rendered unless a real Jinja
> engine (`j2cli`, `envtpl`) is installed — which it isn't here.

Rendered outputs (`.gitconfig`, `.config/ghostty/config`) are **not** tracked:
they're generated by `yadm alt` and listed in [`.gitignore`](.gitignore), so
`yadm status` stays clean regardless of the machine's class or architecture.
